...
* **Initial Remediation (`07cde5a05c176bb519a47d8fc30fb9c6f3c88f95`)**:
  - **Backend (`auth-service`)**: Added `TwoFactorEnabled *bool` to `models.User` (defaults to `true` via `Is2FAEnabled()`). Updated `Login` to check `user.Is2FAEnabled()`; if disabled, login immediately issues JWT session tokens without an OTP challenge. Updated `UpdateProfile` to accept and persist `two_factor_enabled` patches.
  - **Frontend**: Added `twoFactorEnabled` to `UserProfile`, `updateOwnProfile`, and `AuthProvider.toggleTwoFactor(bool enabled)`. Added "Security" card with `Key('settings_two_factor_switch')` in `SettingsScreen`.
  - **Product Safeguard**: Disabling 2FA prompted a confirmation dialog (`ConfirmActionDialog`, `disableTwoFactorConfirmTitle`) warning of reduced account security before disabling.
* **Follow-Up Security Hardening (`42eb73e7897d756b21e24ec84c88b59bda300fac`, rate-limited in `3580fada155c203f6bb79354a702a1b07cc290a7`)**:
  - **Security Gap Addressed**: Closes the gap where an unlocked device alone was sufficient to weaken account security by disabling 2FA without re-authenticating.
  - **Backend (`auth-service`)**: Enforced mandatory password re-verification on the 2FA-disable path (`two_factor_enabled == false`). Verifies password with bcrypt using constant-time dummy hash comparison (`dummyBcryptHash`) for accounts without passwords to maintain timing parity (matching patterns at `auth.go:473`, `:485`, `:814`, `:822`). Missing or invalid passwords return 400/401 and keep 2FA enabled in MongoDB. Consecutive failures trigger lockout rate limiting (429 Too Many Requests). Enabling 2FA remains immediate and requires no password.
  - **Frontend**: In `SettingsScreen`, chained the warning confirmation modal (`ConfirmActionDialog`) to a password input modal (`_DisableTwoFactorPasswordDialog`, `Key('disable_2fa_password_field')`). Shows clear, specific error banner upon incorrect password (`disableTwoFactorPasswordError`) allowing retry without closing the flow; correct password disables 2FA, flips toggle OFF, and displays confirmation snackbar. Password is never logged or persisted client-side and is wiped immediately upon request completion.
  - **Verification**: Verified via `two_factor_password_test.go` (missing pwd 400, wrong pwd 401, timing parity dummy bcrypt comparison, lockout rate limiting, direct enable 200), `settings_screen_test.dart` (warning dialog, password dialog, wrong password error/toggle retention, correct password disable, and direct enable), `gap05_account_status_test.dart`, `my_account_screen_test.dart`, and `golden_screens_test.dart`.
