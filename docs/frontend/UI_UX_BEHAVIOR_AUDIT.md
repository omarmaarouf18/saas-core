...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **`ticket_chat_screen.dart`** | Real-Time / Leaks | **Dead Dispose Hook / Leaking WebSocket & Timer**: Inside `dispose()`, `chat.disconnect()` is wrapped in `addPostFrameCallback((_) { if (mounted) { chat.disconnect(); } })`. In Flutter, `mounted` is always `false` after `dispose()`. Consequently, `chat.disconnect()` is **never called**, leaking the WebSocket connection and exponential reconnect timers in the background after popping the screen. This is a direct regression of defect A6. | `ticket_chat_screen.dart:65-70` | **Critical** | Adopt `ProviderConnectionCleanup<TicketChatScreen>` mixin (as in `chat_screen.dart:28, 44`) and register `addConnectionTeardown` in `initState`. |
| **`rating_screen.dart`** | Auth / Role Lockout | **Customer Rating Hard 403 Rejection**: In `job_status_screen.dart:1175`, customers are prompted to "Rate Your Experience" for completed jobs. In `rating_screen.dart:65-68`, customer rating sets `_otherPartyId = widget.job.employeeId`. However, the backend handler `RateJob` strictly requires `req.RatedBy == job.OwnerID && req.RatedUser == job.EmployeeID` or `req.RatedBy == job.EmployeeID && req.RatedUser == job.OwnerID`. Customer ratings (`req.RatedBy == job.UserID`) are **hard-rejected with 403 Forbidden**. Furthermore, when an employee rates, `rating_screen.dart:61` sets `_otherPartyId = widget.job.ownerId`, rating the business owner rather than the customer. | `rating_screen.dart:65-68` vs `ratings_handlers.go:86-89` | **Critical** | Update backend `RateJob` handler to authorize customer rating courier (`req.RatedBy == job.UserID && req.RatedUser == job.EmployeeID`) and courier rating customer (`req.RatedBy == job.EmployeeID && req.RatedUser == job.UserID`). |
| **`deposit_funds_dialog.dart`** | Error Recovery | **Pseudo-Retry on Deposit Error Banner**: When a wallet deposit fails (e.g., 400 Bad Request, network timeout), `ThemedErrorBanner` renders a "Retry" button. However, `onRetry: () => setState(() => _dialogError = null)` merely clears the banner instead of re-executing `_handleDeposit()`, misleading users into believing they retried the transaction. | `deposit_funds_dialog.dart:130` | **High** | Set `onRetry: _isSubmitting ? null : _handleDeposit`, or omit `onRetry` so the banner functions strictly as an error notification without a false action. |
| **`payout_request_dialog.dart`** | Error Recovery | **Pseudo-Retry on Payout Error Banner**: Tapping "Retry" on payout failure banners at lines 155 and 282 executes `() => setState(() => _dialogError = null)`, clearing the error message without re-submitting the payout request. | `payout_request_dialog.dart:155, 282` | **High** | Wire `onRetry` to `_handlePayout` or omit `onRetry`. |
...
| **`chat_screen.dart` / `chat_provider.dart`** | Real-Time / Data Loss | **Message Deduplication Silently Drops Repeated Text**: In `ChatProvider._handleMessage`, incoming WebSocket messages are deduplicated using: `m.senderId == msg.senderId && m.content == msg.content && m.type == msg.type` without checking timestamp or message ID. Any repeated message sent by the same user (e.g. replying "Yes", "OK", "Thank you", or "On my way" twice in a session) is **silently dropped and never rendered**. | `chat_provider.dart:194-201` | **High** | Deduplicate strictly on `m.id == msg.id` (when IDs exist) or require timestamp proximity threshold (`difference.inSeconds < 2`). |
...
### Tier 1: Critical Core Defects (Resource Leaks, Money Flow, Auth/Rating Lockout)
*Target: Immediate hotfix.*

1. **Fix `ticket_chat_screen.dart` Teardown Leak**:
   - Add `with ProviderConnectionCleanup<TicketChatScreen>` to `_TicketChatScreenState`.
   - In `initState`, register `addConnectionTeardown(Provider.of<ChatProvider>(context, listen: false).disconnect)`.
   - Remove the dead `addPostFrameCallback` in `dispose()`.
2. **Resolve Rating Flow Authorization & Role Assignment**:
   - In Go backend `ratings_handlers.go`: Update `RateJob` authorization to allow customer rating courier (`req.RatedBy == job.UserID && req.RatedUser == job.EmployeeID`) and courier rating customer (`req.RatedBy == job.EmployeeID && req.RatedUser == job.UserID`).
   - In `rating_screen.dart:59-70`: Update `_determineParties()` to correctly bind customer <-> courier relationships.
3. **Correct Pseudo-Retry in Deposit & Payout Dialogs**:
   - In `deposit_funds_dialog.dart:130`: Wire `onRetry: _isSubmitting ? null : _handleDeposit`.
   - In `payout_request_dialog.dart:155, 282`: Wire `onRetry: _isSubmitting ? null : _handlePayout`.
4. **Fix Chat Message Deduplication in `chat_provider.dart`**:
   - Update `_handleMessage`: Check `m.id != null && msg.id != null && m.id == msg.id` for deduplication, or check `m.createdAt.difference(msg.createdAt).inSeconds.abs() < 2` rather than dropping identical text.
