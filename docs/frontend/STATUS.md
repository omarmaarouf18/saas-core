...
* **Phase 15: Unified Account Settings & Role Home Redesign (ADR-0014)** — **[PARTIALLY COMPLETE — SUB-ITEM A VERIFIED]**
    *   *(a) Unified Settings / Account Action Center*: **[VERIFIED]** Implemented `ThemeProvider` (persisting theme mode across restarts via `FlutterSecureStorage`) and `SettingsScreen` (`frontend/lib/screens/settings_screen.dart`). Features fully functional 3-way Light/Dark/System theme selector, role-gated section placeholders ("Owner Configuration" for owner, "My Account" for customer, omitted for employee), and placeholder rows for Language and Customer Service with coming-soon badges and feedback snackbars. Replaced duplicated Logout buttons across `home_screen.dart`, `customer_marketplace_screen.dart`, and `employee_jobs_screen.dart` with a single Settings gear icon navigating to `SettingsScreen`. Verified via widget tests (`frontend/test/settings_screen_test.dart`). Language infrastructure, Owner Configuration content, My Account content, and Customer Service ticket wiring remain explicitly deferred [PLANNED, NOT STARTED].
    *   *(b) Customer Home Redesign*: **[PLANNED, NOT STARTED]** Redesigns the customer home layout to feature a friendly interface with 4 distributed category tiles (exact 4 categories TBD in follow-up scoping conversation), a search bar, and primary navigation entry points.
    *   *(c) Interactive Map-Based Location Picker*: **[PLANNED, NOT STARTED]** Replaces manual latitude/longitude text inputs in `customer_marketplace_screen.dart` with an interactive `flutter_map` location picker for visual location selection and nearby service filtering/sorting.
    *   *(d) Employee Capability Audit & Screen Redesign*: **[PLANNED, NOT STARTED]** Comprehensive audit of all backend capabilities available to the employee role across `user-service`, `auth-service`, and `chat-service` as a mandatory prerequisite task prior to redesigning the employee-facing screen(s).
