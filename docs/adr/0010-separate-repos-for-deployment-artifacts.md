...
4. **Full Decommissioning & Revocation**: Following end-to-end verification of all three GitHub App-driven workflows (`build-and-publish.yml`, `sync-mobile-frontend.yml`, and `build-apk.yml`), legacy PAT secrets (`DEPLOY_REPO_PAT`, `MOBILE_REPO_PAT`, `LOGICLINC_REPO_PAT`) were deleted from repository secrets across all repos and revoked at the GitHub token level (Commit `375a017f9198bc4d747bcd2ba0ab5d542dcc58a9`). Zero live code paths or workflows rely on legacy PATs as of this commit.
