...
3. **Guaranteed-Floor Mechanism**:
   * Recomputed Actual Amount: $A_{\text{actual}} = \text{TenantBasePrice} + (\text{ActualDistance} \times \text{TenantPricePerKM})$.
   * Final Employee Payout: $\max(LockedEscrowAmount, A_{\text{actual}})$.
   * **Worker Protection**: An honest employee who traveled a longer physical route than estimated at booking is paid $A_{\text{actual}}$ (drawing down additional escrow if available or adjusting settlement). The employee is **never paid less than $LockedEscrowAmount$**, ensuring honest workers are never shortchanged by route recalculations.
