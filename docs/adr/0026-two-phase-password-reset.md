...
## Amendment (2026-09-24): bind phase 2 to a possession token

The original decision text above is preserved verbatim for history; what follows supersedes it where they conflict. Review found the residual-risk paragraph understated a real gap: phase 2 authorized on email + `otp_verified` + `otp_expires_at`, with nothing proving the caller is the same party that supplied the correct OTP in phase 1. Anyone knowing only the target's email could complete a reset during the verified window. The fix binds phase 2 to a possession token:

- **Mint (phase 1)**: on successful verification the store mints 32 `crypto/rand` bytes (base64url, 43 chars), stores the SHA-256 hex digest + an independent expiry in the SAME atomic `FindOneAndUpdate` (keyed on the exact OTP ciphertext) that consumes the code — the existing race protection extends to the token fields with no second write. The raw token is returned exactly once, in the verify-code success response (`{"status","message","reset_token"}`); never logged, never persisted. A fresh verify (after resend) overwrites the fields (rotation); `SetOTP` additionally clears them so a resend alone already kills the live token.
- **Token clock**: `ResetTokenTTL = 10 minutes` from successful verification — deliberately NOT `otp_expires_at` (send-time clock abandoned for phase 2; a verified-but-abandoned reset must not stay exploitable for the remainder of the send window).
- **Consume (phase 2)**: `ResetPasswordRequest` gains `reset_token`. The handler hashes the submission and compares with `subtle.ConstantTimeCompare` against the stored digest, plus the token-expiry check. Unknown email, no token issued, wrong token, and expired token all return the SAME generic `invalid or expired reset verification` 401 — and a missing/empty token falls into that same 401 (no distinct 400), so the endpoint never distinguishes "no token" from "bad token". Success clears the token fields in the same update (single-use). `otp_verified` is still SET by the store (shared verify core with the login flow) but no longer authorizes phase 2 — retained as trace, cleared on success as before. IP/email rate limiting is unchanged (orthogonal, already correct).
- **Client**: the Flutter `AuthProvider` holds the token in state (`resetToken`, cleared on reset success); screen 2 navigates on receipt, screen 3 submits it. No static fields; the code is still never carried forward.
- **Contract note**: phase-1 response and phase-2 request shapes changed again (same-repo direct change, no versioning — same reasoning as §3).
