...
### 1. `notification-service` Redis Pub/Sub Self-Delivery Dependency (RESOLVED — QA Audit Q18)
- **Context**: In `notification-service` (`services/notification-service/internal/hub/hub.go`), `Broadcast()` originally published payloads to Redis Pub/Sub channels (`notify:tenant:<id>` and `notify:global`) when Redis was enabled, relying on the instance's own pattern subscription (`PSubscribe("notify:*")`) to loop messages back to locally connected clients.
- **Tradeoff & Failure Mode**: Every notification — including delivery to clients on the originating instance — incurred a full Redis network round-trip. `rdb.Publish()` confirming success only guaranteed Redis received the payload; if the publishing instance's subscriber connection experienced a transient network hiccup or startup-window race, local clients on the origin replica missed the alert without a surfaced error.
- **Resolution**: Resolved via local-first delivery paired with origin instance ID deduplication (matching `chat-service`'s design) and synchronous subscription readiness gating. When `Broadcast(n)` is called, the hub first attempts local delivery in-process immediately (`deliverLocal(n)`) to any client connected to this instance. It also publishes to Redis Pub/Sub tagged with an `OriginInstanceID` envelope (`hubPubSubPayload`). Remote replicas forward the notification to their local clients, while the originating replica identifies its own `OriginInstanceID` and drops the echo to prevent duplicate deliveries. Furthermore, the SSE Stream handler (`/notifications/stream`) verifies that the Redis subscription is confirmed active via `hub.WaitForReady` before completing the handshake (`event: connected`), closing the startup-window subscription race.
