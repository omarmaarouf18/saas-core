...
## Verify-Code Endpoint Rate-Limiting & Anti-Enumeration (ADR-0026)

- **Implementation Detail**: The new `POST /auth/reset-password/verify-code` is a brute-forceable oracle by construction (6-digit codes), so it carries the full `ResetPassword`-grade protection set, each pinned by its own test: dual-keyed `AuthRateLimiter` checks (`IsLocked` IP then email) FIRST, before any OTP comparison, on the same `a.limiter` instance (no second limiter); `RecordFailure` on both keys for wrong/expired/unknown; `Reset` on both keys for correct; byte-identical generic 401 (`invalid or expired OTP code`) for user-not-found vs wrong-code vs expired-code; success body verified field-by-field to carry no token/session/user. Threat model note (also in the ADR): the rate limiter — not code consumption — is the brute-force defense (a wrong guess provably leaves the code verifiable afterward); consumption additionally gives at-most-once verification (replay fails). The phase-2 gate (`otp_verified` + `otp_expires_at`, one generic 401) and the removal of the old distinguishable 404 branch are covered in the companion `new-features.md` entry — this entry exists separately because authorization-boundary wiring deserves its own audit trail, not because the feature entry omits it.
- **Commit SHA**: ``ccddf4e24b406d09c28d33e769c469c00a0d6659``
- **Verification**: 10 distinct cases in `services/auth-service/internal/handlers/reset_password_verify_code_test.go` (6th-wrong→429, non-consumption, counter-reset, IP-lockout-before-comparison, email-lockout, uniform-errors, no-token-body, replay, 3-subcase phase-2 gate); full `auth-service` suite green.
...
- **Commit SHA**: ``1ff251a8b1fe323e973d7a33ce6356d97a421dfd``
...
- **Commit SHA**: ``5cddfb2e92f3071fad74716975d4a2661c6a2be7``
...
- **Commit SHA**: ``617e26396b9f1e34e6269cc56e7b39834ae2ebb9``
...
- **Commit SHA**: ``f4c29f1b8d102d95d471f2851db810c0d17d85ea``
...
- **Commit SHA**: ``d0cb4b2a0d938dd778dbbb8fb22d9bd01085361d``
...
- **Commit SHA**: ``dba25a27aab3c21adf99de6e4458bfb41206d5d1``
...
- **Commit SHA**: ``9167d87485430d872bbe24b28241ec356808a6a0``
...
- **Detail**: Two store test files updated by the GetLedger/GetRatings signature change were left unstaged in commit `617e26396b9f1e34e6269cc56e7b39834ae2ebb9`; committed separately as `3e88844cd5893848c9e0d52280fcec7fda5593db` (test-only, mechanical `(0,0)` default-page args). Recorded for transparency per repo honesty rules.
...
- **Commit SHA**: ``9ab8ef3d9269ec95fe511233f5531165319e1488``
...
- **Commit SHA**: ``2bd3755d99f094db5539a88003e982726bc87922``
...
## Atomic OTP Consumption via Ciphertext CAS (QA Audit Q7)

- **Implementation Detail**: Independent QA audit finding. `MongoDB.VerifyOTP` (`services/auth-service/internal/store/mongodb.go`) validated the submitted code against a snapshot read and then performed an UNCONDITIONAL consume write filtered only by `{email}`. Consequences: (1) N concurrent submits of one valid code all passed the compare and all succeeded — repro showed **12 of 12 workers consuming the same OTP in every round**, i.e. twelve login JWTs from a single 2FA code; (2) a concurrent `SetOTP` (resend flow) interleaving with an in-flight verify was overwritten on stale evidence — the newest issued code destroyed, superseded codes still accepted. The consume is now an optimistic CAS: `FindOneAndUpdate` filtered on `{email, otp_code: <exact ciphertext validated>}`. AES-GCM's per-encryption random nonce makes every issued code's ciphertext unique, so the stored value doubles as a one-time handle: resends supersede it, competing verifiers invalidate it. Generic `invalid OTP` rejection preserved for anti-enumeration; at-rest crypto posture unchanged (no plaintext-equivalent hash introduced).
- **Commit SHA**: ``457037cd1bb81c01d04833d3fe96fdd0a65230f7``
- **Verification**: New concurrency regression `TestVerifyOTP_ConcurrentSubmitConsumesExactlyOnce`: 12 workers × 8 rounds barrier-started against one valid code, asserting exactly one success per code. Pre-fix literal failure: `round 0..7: OTP consumed 12 times` → `OTP CONSUMED MULTIPLE TIMES BY CONCURRENT SUBMITS OF ONE CODE across 8 rounds`. Post-fix: 5/5 consecutive `-count=5` passes with zero multi-consumptions; full auth-service suite green (`go test ./... -count=1`: config/handlers/models/otp/otpcrypto/storage/store all `ok`, including existing login-2FA and reset-password flows). ✅

## GetRatings Raw-ID Fall-Through Restricted by Target Role (QA Audit Q24)

- **Implementation Detail**: Same alias-shadowing class as the fixed GetJob bug: `GetRatings` (`services/user-service/internal/handlers/ratings_handlers.go`) validated the requester's JWT and then discarded the claims; a target `user_id` failing JWT resolution fell through as the raw lookup key, so any authenticated principal could read any user's rating history including reviewer identities and comment text (pre-fix literal repro: `RAW CUSTOMER TARGET LEAKED: status=200, ratings=2 for unresolved target 'cust-victim-raw'`). Contract decision C': business reputation stays publicly queryable by raw ID — the ADR-0014 directory/marketplace passes `widget.tenantId` as a raw owner ID and blanket rejection would break that product flow — while customer rating histories (private blind-feedback data) now require the customer's own resolvable token. Unresolved raw targets are classified via new nil-safe `fetchUserRole` internal call; customer/unknown targets receive 403. RateJob's analogous `rated_user` loose fallback was independently verified contained by pair-equality authz against server-stored job parties and left unchanged. Legacy big-suite mock extended with an `emp-` prefix branch so raw employee targets classify correctly.
- **Commit SHA**: ``df21c6ccb65979d283486a1cb9adc43aea7fd9db``
- **Verification**: New regression test `TestGetRatings_RawCustomerTargetRejected_BusinessTargetsStayPublic` (role-aware mock harness): pre-fix literal failure quoted above; post-fix 3/3 passes asserting (1) raw owner target still 200 (marketplace contract), (2) raw customer target 403, (3) customer's own token still 200 for their history. Full user-service suite green after two follow-ups the fix surfaced: nil-safe guard for hand-built harnesses (nil `authClient` previously panicked in `read_rate_limiters_test`), and the legacy mock extension. ✅
- **Follow-up (CI blocker, pre-push)**: CI's `gosec@latest` gained a taint rule (G704, "SSRF via taint analysis", CWE-918) that flags `fetchUserRole`'s `http.NewRequest` — the request-derived ratings `targetUserID` reaches the internal lookup URL as tainted data. The suppression added by ``e6ac002`` sat on the `authClient.Do` line while gosec attributes the finding to `NewRequest`, so the marker never engaged and a mid-flight working-tree REVERT of this entire fix was drafted on that basis (rationale claimed evidence in `scratch/qa_audit/`, which contains no such evidence; sibling sites `checkKYC`/`verifyEmployeeAssignment` use the identical call shape but are unflagged because their IDs are not request-tainted). Resolution: suppression relocated onto the flagged sink per repo `#nosec` convention (host is trusted internal config; ID is `url.QueryEscape`'d into a query parameter of our own auth-service); verified `gosec -quiet ./...` exits 0 and the revert was discarded. Commit ``c12580a``.

## Empty-Secret Guards on All X-Internal-Token Comparators (QA Audit Q23)

- **Implementation Detail**: Independent QA audit finding. Eight comparators across four services authenticated internal callers via `subtle.ConstantTimeCompare` with no empty-config check; since `ConstantTimeCompare("","")==1`, any handler constructed with a blank configured token treated every bearerless request as an internal service call. Worst site: user-service `CompleteJob` — role checks skipped, escrow release / COD settlement reachable (reproduced live pre-fix: `EMPTY-SECRET INTERNAL AUTH ACCEPTED: CompleteJob settled job "job-q23-complete" with no token and no requester identity when the configured secret is empty`). Loaders fail-fast on missing env today; these guards pin the invariant so a harness/refactor bypassing config.Load() cannot reopen unauthenticated fund movement or forged broadcasts. Guard pattern mirrors the two pre-existing correct auth-service sites: reject when configured secret is empty OR comparison fails.
- **Sites covered**: user-service CompleteJob / GetJob (raw doc read) / CancelJob; chat-service BroadcastLocation; notification-service Send + BroadcastJobAlert; api-gateway /health/internal + admin version-config PUT.
