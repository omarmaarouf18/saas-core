...
2. **`chat-service/internal/handlers/chat.go`**: Verify that query-parameter authentication tokens (`?token=`) for WebSocket connections are securely cleared/redacted on client handshake errors before logging. **[RESOLVED-BY-VERIFICATION 2026-08-22]** — handshake/error paths in `chat.go` contain no URL or RawQuery logging (grep-verified); there is nothing to redact. Flag closed as verified-moot.
