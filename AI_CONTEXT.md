...
* **Verified Token Role Gating in resolveTokenWithRole (Item #4)**: Added `resolveTokenWithRole` helper supporting variadic allowed roles (`owner`, `employee`, `user`, etc.), rejecting tokens with mismatched roles early rather than relying solely on downstream checks. Verified via `TestResolveTokenWithRole`.
