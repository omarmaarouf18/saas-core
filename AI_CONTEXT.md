...
* **Verified Resend Email OTP Delivery End-to-End (`logiklinkeg.tech` / `ResendDispatcher`)**: Real end-to-end email OTP dispatch was verified against live Resend API infrastructure using `ResendDispatcher` (`github.com/resend/resend-go/v3`) configured for domain `logiklinkeg.tech` in `auth-service`. Verified full lifecycle: user signup (`POST /api/v1/auth/signup`) dispatched live email via Resend API, suppressed `dev_otp` in non-local mode (`APP_ENV=production`), stored AES-256-GCM encrypted OTP ciphertext at rest in MongoDB, delivered email OTP, completed 2FA authentication via `POST /api/v1/auth/verify-otp` (HTTP 200 OK + JWT token issued), and confirmed graceful error logging on invalid API keys without crashing the service.
...
* **Verified Negotiable Transport Pricing Escrow Locking & Reconciliation Fallback (`RespondPrice`)**: `RespondPrice` (`POST /users/jobs/respond-price`) locks escrow for non-COD transport jobs upon price proposal acceptance (`decision == "accept"`), calling `LockEscrow` and persisting `LockedEscrowAmount` on the job record before transitioning status to `active`. If `UpdateJobLockedEscrow` fails, `performRollbackEscrow` executes a single retry; if rollback itself fails, `UpdateJobReconciliation` transitions the job to `escrow_reconciliation_required` with `ReconciliationNote` set, preserving the job for owner resolution via `GET /users/jobs/reconciliation-queue`. Verified via full lifecycle state audit test (`Test1_FullLifecycleStateAudit`) verifying zero drift between job escrow and wallet debit/release balances, cross-feature interaction test (`Test3_CrossFeatureInteraction_RespondPrice_And_ADR0007_SpeedCheck`), and table-driven test suite (`TestRespondPrice_EscrowLockingAndReconciliationFallback` 6/6 pass). (Commit `e8eb530ebcd0529369596d87f807052b55ff7d8a`).
