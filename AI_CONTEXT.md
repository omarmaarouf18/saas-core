...
* **Verified Token Role Gating in resolveTokenWithRole (Item #4 Remediation)**: Replaced bare `resolveToken` pass-through calls in `CreateService` and `TrackJob` with explicit role requirements (`resolveTokenWithRole(ownerID, "owner")`, `resolveTokenWithRole(userID, "user", "customer")`, `resolveTokenWithRole(employeeID, "employee")`), returning 401 Unauthorized on role mismatch. Verified via `TestRoleEnforcement_CreateServiceAndTrackJob`.
