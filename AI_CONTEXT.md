...
* **Verified Markdown Commit SHA Non-Blocking Warning Pass**: Extended markdown commit SHA verification in `.githooks/pre-push` and `.github/workflows/ci.yml` to include a second non-blocking pass for any 40-character hex string in prose outside citation lines, printing `WARNING:` without blocking push/CI execution. Verified via `make docs-check` and pre-push validation. (Commit `ea9f52add3561df24d008699ee24187139a56bde`).
