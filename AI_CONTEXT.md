...
* **Known Open Item: Deferred Escrow Lock in Negotiable Transport Pricing (`RespondPrice`)**: When a negotiable transport category job price is accepted via `POST /users/jobs/respond-price` (`RespondPrice`), `job.AgreedPrice` is set to the accepted price and status transitions to `active`, but no escrow balance lock (`LockedEscrowAmount`) or wallet deduction is executed at agreement time. Subsequent escrow settlement via `CompleteJob` relies on `LockedEscrowAmount`, which remains `0.0` for negotiated transport jobs. Escrow locking upon price agreement remains an open gap.
